...
export default function PropertyOwnerEstimatesPage() {
  ...
  const handleApprove = async (item: EstimateItem) => {
    if (!accessToken) return;
    try {
      await approveEstimate({ token: accessToken, id: item.id });
      addToast({
        variant: 'success',
  ...
  const handleReject = async (item: EstimateItem) => {
    if (!accessToken) return;
    try {
      await rejectEstimate({ token: accessToken, id: item.id });
      addToast({
        variant: 'success',
