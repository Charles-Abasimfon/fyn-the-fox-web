...
const ResolvedWorkOrdersPage = () => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      // Filter for resolved/completed complaints
      const resolvedComplaints = resp.complaints.filter((c) => {
        const status = c.status.toLowerCase();
        return status === 'completed';
      });
      setWorkOrders(resolvedComplaints.map(mapComplaintToWorkOrder));
    } catch (e: any) {
      const msg =
        e instanceof ApiError
          ? e.message
          : 'Failed to load resolved work orders';
      setError(msg);
    } finally {
      setLoading(false);
    }
    ...
    <div className='py-6 pt-8'>
      <div className='mb-6'>
        ...
        <p className='text-sm text-white/60 mt-1'>
          View all completed work orders
        </p>
      </div>
      {vendorsLoading && (
...
export default ResolvedWorkOrdersPage;
